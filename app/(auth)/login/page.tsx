  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#f8f9f8' }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 p-12"
           style={{ background: '#0c1714' }}>
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center"
               style={{ background: '#16a37f', boxShadow: '0 2px 12px rgba(22,163,127,.4)' }}>
        ...
        <div>
          <blockquote className="text-[22px] font-light text-white leading-relaxed tracking-tight mb-8"
                      style={{ letterSpacing: '-0.3px' }}>
            "Property management software that works the way you do — not the other way around."
          </blockquote>
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: 'Properties', value: '400+' },
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-[400px]">
          ...
          <p className="text-[13px] mb-8" style={{ color: '#8a9889' }}>
            {isSignUp ? 'Start managing your properties for free.' : 'Sign in to your portfolio.'}
          </p>

          {/* Mode toggle */}
          <div className="flex rounded-lg p-1 mb-6" style={{ background: '#f0f0ef', border: '1px solid #e4e7e4' }}>
            <button onClick={() => setMode('password')}
                    className="flex-1 py-2 text-[12.5px] font-medium rounded-md transition-all"
                    style={{
                      background: mode === 'password' ? 'white' : 'transparent',
                      color: mode === 'password' ? '#111612' : '#8a9889',
                      boxShadow: mode === 'password' ? '0 1px 3px rgba(0,0,0,.08)' : 'none'
                    }}>
              Password
            </button>
            <button onClick={() => setMode('magic')}
                    ...
                    style={{
                      ...
                      color: mode === 'magic' ? '#111612' : '#8a9889',
                      boxShadow: mode === 'magic' ? '0 1px 3px rgba(0,0,0,.08)' : 'none'
                    }}>
              Magic Link
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] font-medium mb-1.5" style={{ color: '#4a5549' }}>
                Email address
              </label>
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                     ...
                     style={{
                       background: '#f8f9f8', border: '1px solid #e4e7e4',
                       fontFamily: 'inherit', color: '#111612'
                     }}
            ...
            {message && (
              <div className="rounded-lg px-3 py-2.5 text-[12.5px]"
                   style={{
                     background: message.type === 'success' ? '#f0fdf9' : '#fef2f2',
            ...
            <button type="submit" disabled={loading}
                    ...
                    style={{
                      background: loading ? '#7fa89e' : '#16a37f',
                      boxShadow: loading ? 'none' : '0 1px 4px rgba(22,163,127,.3)',
          ...
          <p className="text-center text-[11px] mt-8" style={{ color: '#b0bcb0' }}>
            Your data is encrypted and isolated per account.
          </p>
        </div>
      </div>
    </div>
