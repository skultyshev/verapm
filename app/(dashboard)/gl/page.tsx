...
export default async function GLPage() {
  ...
  const [{ data: entriesRaw }, { data: accounts }] = await Promise.all([
    supabase
      ...
      .select('*, lines:journal_lines(id, debit, credit, description, account:accounts(account_number, name, type))')
      .eq('is_void', false)
    ...
    <>
      <div className="bg-white border-b border-gray-200 px-6 h-14 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/gl/new-entry" className="inline-flex items-center gap-1.5 px-3.5 h-[34px] rounded-md text-[12.5px] font-medium text-white" style={{ background: '#16a37f' }}>
            + New Entry
          </Link>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        <div className="grid grid-cols-5 gap-3">
          {(['asset','liability','equity','revenue','expense'] as const).map(type => {
            const typeAccs = (accounts as any[])?.filter(a => a.type === type) || []
              ...
              {entries.map((je: any) => {
                ...
                return (
                  <tr key={je.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 cursor-pointer">
                    ...
                    <td className="px-4 py-3 text-[12px] text-gray-500">{je.property_id ? je.property_id : 'Portfolio'}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={[
                        'badge text-[10.5px] px-2 py-0.5 rounded-full font-semibold',
                        je.is_posted && bal ? 'bg-green-50 text-green-700 border border-green-200'
                        : je.is_posted ? 'bg-red-50 text-red-700 border border-red-200'
                    ...
                    <td className="px-4 py-3">
                      <Link href={'/gl/' + je.id} className="text-[11px] px-2.5 py-1.5 rounded border border-gray-200 text-gray-500 hover:bg-gray-50 transition-all">
                        View
                      </Link>
                    </td>
                  </tr>
